...
**Critical**: Bound text inside a shape MUST always use `"textAlign": "center"` and `"verticalAlign": "middle"`. Never use `"left"`, `"right"`, `"top"`, or `"bottom"` for text that lives inside an element.
...
9. All bound text (text inside a shape) uses `"textAlign": "center"` and `"verticalAlign": "middle"`
